...
import { ArrowLeft, Users, Search, RefreshCw, ChevronDown, Check, CheckCircle2, Clock, ExternalLink, AlertCircle } from 'lucide-react';
...
export default function Assign() {
    ...
    const loadData = async (token) => {
        ...
        try {
            ...
            const catalogData = (assignmentsResult.catalog || []).filter(item =>
                                           ...
                                           return (
                                               <>
                                                   <span className="font-medium">
                                                       {chapter ? `Chapter: ${chapter}` : (item.contentType === 'quiz' ? 'Quiz' : 'Lesson')}:
                                                   </span>{' '}
                                                   {chapter ? `· ${item.title}` : item.title}
                                               </>
                                           );
                                       })()}
