...
Deno.serve(async (req) => {
    try {
        ...
        const { csvUrl, studentEmailFilter } = await req.json();
        ...
        for (const row of rows) {
            try {
                ...
                if (studentEmailFilter && studentEmail !== studentEmailFilter.toLowerCase().trim()) {
                    skipped++;
                    continue;
                }
                ...
                const dedupeKey = await hashString(
                    `csv-historical-quiz-import:${studentEmail}:${quizName}:${dateCompleted}`
                );
                ...
                await base44.asServiceRole.entities.ActivityEvent.create({
                    ...
                    thinkificUserId: 0,
                    studentDisplayName: row['Student Name'] || studentEmail.split('@')[0],
                    ...
                    metadata: {
                        ...
                        importSource: 'historical_csv_import'
                    }
                });

                imported++;
                
                // Track per-student summary
                if (!studentsSummary[studentEmail]) {
                    studentsSummary[studentEmail] = 0;
                }
                studentsSummary[studentEmail]++;
        ...
        return Response.json({
            ...
            errors,
            studentsSummary,
            message: `Imported ${imported} quiz results for ${Object.keys(studentsSummary).length} student(s)`
        });
