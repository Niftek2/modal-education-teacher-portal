...
async function createThinkificUser(firstName, lastInitial, email) {
    ...
    if (!res.ok) {
        ...
        throw new Error(err.message || err.errors?.[0]?.message || 'Failed to create user');
    }
...
Deno.serve(async (req) => {
    try {
        ...
        const { teacherEmail: rawTeacherEmail, groupId, students } = await req.json();
        ...
        for (const student of students) {
            try {
                ...
                await Promise.all(ALL_COURSE_IDS.map(courseId => enrollInCourse(user.id, courseId)));
